...
public class Interaction extends MyFarmPanel {
   public static void capturingPlayer()
   {
      ...
      if (captured)
      {
         playerR = closeR;
         playerC = closeC;
         character[playerR][playerC].setCaptured(true);
         
         //TURN OF CHASE MODE
         character[closeR][closeC].setTypeOfMovement(0);
      }
         
      if (Math.abs(closeR-playerR) + Math.abs(closeC-playerC) == 1 && !justReleased && capturedPlayer == null) //if they are next to each other
      {
         capturedStatus = 5;
         captured = true;
         ...
         capturedPlayer.setCaptured(true);
      }
      ...
      if (capturedStatus == 0)
      {
         ...
         character[playerR][playerC].setCaptured(false);
      }
   }
}